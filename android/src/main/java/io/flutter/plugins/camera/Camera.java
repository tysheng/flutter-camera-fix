...
public class Camera {
    ...
    private CameraFix cameraFix = new CameraFix();
...
//      previewSize = computeBestPreviewSize(cameraName, preset);
        Log.d(TAG, "Camera: captureSize " + captureSize);
        previewSize = cameraFix.previewSize(getIdForBackCamera());
        Log.d(TAG, "Camera: previewSize " + previewSize);
    }
     ...
     *
     */
    private static int getIdForBackCamera() {
    ...
    private Size rewritePreviewSize(Size size) {
        int maxLength = Math.max(size.getHeight(), size.getWidth());
        float ratio = maxLength * 1f / Math.round(maxLength * 0.8f);
        return new Size(Math.round(size.getWidth() * 1f / ratio), Math.round(size.getHeight() * 1f / ratio));
    }
